...
public class HtmlSaver {
    ...
    public static void main(String[] args) {
        ...
        try (FileWriter writer = new FileWriter(OUTPUT_FILE)) {
            ...
            for (int i = 0; i + 1 < newsParent.size(); i += 2) {
                Element newsItem = (Element) newsParent.get(i + 1);
                Elements dates = newsItem.getElementsByClass("blockdate");
                Elements titles = newsItem.getElementsByClass("blocktitle");
                if (dates.isEmpty() || titles.isEmpty()) {
                    continue;
                }

                String date = dates.get(0).text();
                String topic = titles.get(0).text();
